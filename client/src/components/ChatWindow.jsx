...
const ChatWindow = ({ socket }) => {
  ...
  const { markRoomRead, incrementUnread } = useNotif()
  ...
  const handleNewMessage = useCallback((msg) => {
    ...
    const isCurrentConv = msg.conversationId === activeConv?.id
    if (isCurrentRoom || isCurrentConv) {
      addMessage(msg)
      if (isCurrentConv && msg.senderId !== user.id) {
        socket?.emit('message:read', { messageId: msg.id, conversationId: activeConv.id })
      }
    } else {
      if (msg.roomId) incrementUnread(msg.roomId)
    }
  }, [activeRoom, activeConv, addMessage, incrementUnread, socket, user.id])
  ...
  useEffect(() => {
    ...
    socket.on('message:read_receipt', ({ messageId }) => {
      updateMessage({ id: messageId, readAt: new Date().toISOString() })
    })
    return () => {
  ...
  useEffect(() => {
    if (!activeRoom && !activeConv) return
    if (activeRoom) {
    ...
    if (activeConv) {
      socket?.emit('conv:join', activeConv.id)
  ...
  const getSubtitle = () => {
    if (activeRoom) return activeRoom.description || null
    if (activeConv) {
      const other = activeConv.participants?.find(p => p.id !== user.id)
      return `Direct message with ${other?.username}`
    }
    return null
  }
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#0f1117] min-w-0">
      <div className="px-6 py-4 border-b border-slate-800 flex-shrink-0">
        ...
        {getSubtitle() && <p className="text-slate-500 text-xs mt-0.5">{getSubtitle()}</p>}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-1">
        ...
        {!loadingMessages && messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center py-16">
            <p className="text-slate-600 text-sm">No messages yet. Say something!</p>
          </div>
        )}
        {messages.map(msg => (
